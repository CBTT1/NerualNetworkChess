...
def nn_applygradient(nn):
    ...
    for k in range(nn.depth - 1):
        if nn.batch_normalization == 0:
            ...
            elif method == 'RMSPropNM':
                rho = 0.9  # rho = 0.9
                alpha = 0.9
                nn.rW[k] = rho * nn.rW[k] + (1 - rho) * nn.W_grad[k] ** 2
                nn.rb[k] = rho * nn.rb[k] + (1 - rho) * nn.b_grad[k] ** 2
                nn.vW[k] = alpha * nn.vW[k] - (nn.learning_rate / (np.sqrt(nn.rW[k]) + 0.001)) * nn.W_grad[k]
                nn.vb[k] = alpha * nn.vb[k] - (nn.learning_rate / (np.sqrt(nn.rb[k]) + 0.001)) * nn.b_grad[k]
                nn.W[k] = nn.oW[k] + nn.vW[k]
                nn.b[k] = nn.ob[k] + nn.vb[k]  # rho = 0.9
            ...
            if method == 'normal':
                ...
                rho = 0.9  # RMSProp decay rate
                alpha = 0.9  # Nesterov momentum coefficient
                ...
                nn.vW[k] = alpha * nn.vW[k] - (nn.learning_rate / (np.sqrt(nn.rW[k]) + 0.001)) * nn.W_grad[k]
                nn.vb[k] = alpha * nn.vb[k] - (nn.learning_rate / (np.sqrt(nn.rb[k]) + 0.001)) * nn.b_grad[k]
                nn.vGamma[k] = alpha * nn.vGamma[k] - (nn.learning_rate / (np.sqrt(nn.rGamma[k]) + 0.001)) *  nn.Gamma_grad[k]
                nn.vBeta[k] = alpha * nn.vBeta[k] - (nn.learning_rate / (np.sqrt(nn.rBeta[k]) + 0.001)) * nn.Beta_grad[k]
                nn.W[k] = nn.oW[k] + nn.vW[k]
                nn.b[k] = nn.ob[k] + nn.vb[k]
